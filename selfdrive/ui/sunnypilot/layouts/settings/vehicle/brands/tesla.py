...
class TeslaSettings(BrandSettings):
  def __init__(self):
    ...
    self.speed_sync_toggle = toggle_item_sp(tr("Tesla Speed Sync (Experimental A/B)"), "",
                                            param="TeslaSpeedSync")
    self.items = [self.coop_steering_toggle, self.long_fusion_toggle, self.curve_assist_toggle, self.speed_sync_toggle]
  ...
  def update_settings(self):
    ...
    speed_sync_desc = (
      f"{tr('Experimental A/B option. openpilot cruise target follows Teslas own set speed, so both systems')}<br>" +
      f"{tr('share one target. Adjust speed with the Tesla stalk/screen; road-aware changes apply to openpilot too.')}"
    )
    ...
    if not ui_state.is_offroad():
      ...
      speed_sync_desc = f"<b>{enable_offroad_msg}</b><br><br>{speed_sync_desc}"
    ...
    self.speed_sync_toggle.set_description(speed_sync_desc)
    ...
    self.speed_sync_toggle.action_item.set_enabled(ui_state.is_offroad())
